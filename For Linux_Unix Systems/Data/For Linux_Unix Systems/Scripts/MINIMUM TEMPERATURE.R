...
TN_split |>
  ...
  .[order(.[ ,Station]), ] -> startEndYears_TN

# Writing to Disk
write.csv(
  startEndYears_TN,
  file = "For Linux_Unix Systems/outputs/TMin/StartEndYears.csv",
  row.names = FALSE
)
...
write.csv(
  Profile_StationsTN, 
  file = "For Linux_Unix Systems/outputs/TMin/Profiled StationsTN.csv", 
  row.names = FALSE
)
...
Duplicates_TN <- list(
  ...
  TN_ID_Duplicates = {Profile_StationsTN |> . =>
      .[duplicated(.[ ,"ID"]), ][ ,"ID"] |> 
      unlist() -> dupsID_TN
  ...
  OneTown_SameORdiffID_DiffType = {Profile_StationsTN |> . =>
      .[duplicated(.[ ,"StationName"]), ][ ,StationName] -> dupsName_TN
    ...
    split(a, as.factor(a$StationName)) -> ac
